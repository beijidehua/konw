...
// 3. 核心方法：获取知识库详情数据
const fetchKbData = async (id: string) => {
  try {
    // 重置状态：开始加载
    loading.value = true;
    errorMsg.value = '';
...
onMounted(() => {
  // 验证 kbId 是否存在（防止非法访问）
		ElMessage.warning(`id=${id}`)
  if (!kbId.value || kbId.value === 'undefined' || kbId.value === 'null') {
    ElMessage.warning('无效的知识库ID');
    router.push('/knowledge_edit'); // 跳回知识库列表页
    return;
  }
  // 加载知识库数据
  fetchKbData(kbId.value);
});

// 5. 监听路由参数变化，支持同页面切换不同知识库
watch(
  () => route.params.id,
  (newId) => {
...
// 6. 辅助方法：返回知识库列表页
const handleGoBack = () => {
  // 返回到上一页，如果上一页不存在则跳转到列表页
  router.back() || router.push('/knowledge_edit');
};
</script>
